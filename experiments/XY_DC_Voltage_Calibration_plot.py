...
# # XY DC 电压标定 — 数据分析与可视化
#
# **无需连接任何仪器**，仅读取本地 `data/XY_DC_Voltage_Calibration/<run>/`
# 下的原始数据进行分析。
#
# ## 数据结构
# - 数据目录: `data/XY_DC_Voltage_Calibration/<MMDD_HHMM_xy_dc_cal>/`
# - 原始数据 (按 XY DC 电压分组):
...
# 3. 拟合 `f_peak_Hz = K_eff * XY_DC_VOLTAGE_V + B_eff`
#
# ## 输出
# - `results/analysis.yaml`、`results/analysis.json`
# - `results/xy_dc_calibration_results.npz`
# - 至少 4 张图：
#   - `xy_dc_calibration_curve.png` (f_peak vs XY_DC_VOLTAGE + 线性拟合)
#   - `xy_dc_calibration_residual.png` (拟合残差)
#   - `frequency_response_examples.png` (典型 XY DC 电压下的响应曲线)
...
# %% Cell 2
# ========== 配置与数据选择 ==========
EXPERIMENT_TYPE = "XY_DC_Voltage_Calibration"

USE_LATEST = True
DATA_DIR_OVERRIDE = None
# DATA_DIR_OVERRIDE = "data/XY_DC_Voltage_Calibration/0701_1200_xy_dc_cal"
...
if DATA_DIR_OVERRIDE:
    ...
    DATA_DIR = project_root / "data" / EXPERIMENT_TYPE / "MMDD_HHMM_xy_dc_cal"
...
XY_DC_LIST_CONFIG = (config.get("xy_dc_voltage_scan", {})
                     .get("XY_DC_VOLTAGE_LIST_V", None))
...
# %% Cell 6
# ============================================================
# 对每个 XY DC 电压点：加载所有 Z 频点数据 → 提取 FFT + lock-in → 拟合
# ============================================================
print("\n" + "=" * 60)
print("开始对每个 XY DC 电压点提取响应曲线与拟合")
print("=" * 60)
...
# %% Cell 7
# ============================================================
# 线性标定：f_peak_Hz = K_eff * XY_DC_VOLTAGE_V + B_eff
# ============================================================
print("\n" + "=" * 60)
print("线性标定: f_peak_Hz = K_eff * XY_DC_VOLTAGE_V + B_eff")
print("=" * 60)


def linear_calibration(x_arr, y_arr, min_points=LINEAR_FIT_MIN_POINTS,
                       x_label="XY_DC_VOLTAGE_V", y_label="f_peak_Hz"):
...
# ---- 图 1: xy_dc_calibration_curve.png ----
# f_peak vs XY_DC_VOLTAGE, 含线性拟合
...
ax1.set_xlabel("XY DC voltage (V)")
ax1.set_ylabel("f_peak (Hz)")
ax1.set_title("XY DC voltage calibration — f_peak vs XY_DC_VOLTAGE")
ax1.legend(fontsize=8, loc="best")
ax1.grid(False)
plt.gcf().set_layout_engine("constrained")
save_figure(fig1, results_dir / "xy_dc_calibration_curve.png", close=False)
plt.show()
print(f"已保存: xy_dc_calibration_curve.png")

# ---- 图 2: xy_dc_calibration_residual.png ----
...
ax2.set_xlabel("XY DC voltage (V)")
ax2.set_ylabel("Residual (Hz)")
ax2.set_title("XY DC voltage calibration — Linear fit residual")
if plotted_any:
    ax2.legend(fontsize=8, loc="best")
ax2.grid(False)
plt.gcf().set_layout_engine("constrained")
save_figure(fig2, results_dir / "xy_dc_calibration_residual.png", close=False)
plt.show()
print(f"已保存: xy_dc_calibration_residual.png")

# ---- 图 3: frequency_response_examples.png ----
# 选 3-4 个代表性 XY DC 电压点画响应曲线
...
ax4.set_xlabel("XY DC voltage (V)")
...
# %% Cell 9
# ============================================================
# 保存 analysis.yaml / analysis.json / xy_dc_calibration_results.npz
# ============================================================
print("\n" + "=" * 60)
print("保存分析结果")
print("=" * 60)
...
analysis = {
    ...
    "purpose": (
        "标定 XY_DC_VOLTAGE 与响应中心频率 f_peak 的线性关系，"
    ...
    "linear_calibration": {
        "formula": "f_peak_Hz = K_eff * XY_DC_VOLTAGE_V + B_eff",
        # ---- source 说明 ----
        "primary_method": "offline_lockin",
        "primary_peak_source_policy": PEAK_STRATEGY,
        # ---- 各方法线性拟合 ----
...
npz_path = results_dir / "xy_dc_calibration_results.npz"
...
if lin_fit_li.get("success"):
    ...
    print(f"   f_peak_Hz = ({K:+.4f} ± {lin_fit_li['K_eff_unc']:.4f}) * "
          f"XY_DC_VOLTAGE_V + ({B:+.4f} ± {lin_fit_li['B_eff_unc']:.4f})")
    ...
    print(f"   XY_DC_VOLTAGE_V = (TARGET_FREQ_Hz - B_eff) / K_eff")
    print(f"   例如: TARGET_FREQ_Hz = 2000 -> "
          f"XY_DC_VOLTAGE_V = {(2000 - B) / K:.4f} V")
else:
    print("⚠ 离线锁相线性标定失败")
