# %% [markdown] Cell 0
# # AWScale 常数 Omega_ctrl 标定 sanity check — 离线分析
#
# 无需连接任何仪器。读取 `AW_ConstOmega_Calibration_Check_AWScale.py` 保存的 raw 数据，
...
def analyze_single_trace(path):
    with np.load(path, allow_pickle=True) as data:
        ...
        awx = float(scalar(data["aw_normalized_x"])) if "aw_normalized_x" in data.files else None
        awy = float(scalar(data["aw_normalized_y"])) if "aw_normalized_y" in data.files else None
...
fig1.tight_layout()
response_path = results_dir / "const_awscale_response_curves.png"
...
ax2.set_title("Constant AWScale Omega Calibration Residual")
ax2.grid(True, alpha=0.3)
ax2.legend(fontsize=9)
fig2.tight_layout()
residual_path = results_dir / "const_awscale_residuals.png"
...
for rec in traces:
    table_rows.append({
        ...
        "aw_normalized_x": rec["aw_normalized_x"],
        "aw_normalized_y": rec["aw_normalized_y"],
...
summary_json_path = results_dir / "const_awscale_omega_check_summary.json"
with open(summary_json_path, "w", encoding="utf-8") as f:
    json.dump({"records": table_rows}, f, indent=2, ensure_ascii=False)
print(f"兼容 summary 已保存: {summary_json_path}")

csv_path = results_dir / "const_awscale_peak_table.csv"
