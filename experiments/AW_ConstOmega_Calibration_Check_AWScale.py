# %% [markdown] Cell 0
# # AWScale 常数 Omega_ctrl 标定 sanity check
#
# 用同一条 AW -> external AM -> XY 旋转场链路输出常数 Omega_ctrl，短扫 Z RF 频率，
# 比较固定 AW offset=0、amplitude=2 Vpp 时，通过改变任意波常数值实现等效 DC 的标定结果。
#
# 本脚本只负责连接仪器、执行短扫并保存 `raw/` 原始数据；峰值、残差和图表由
# `experiments/AW_ConstOmega_Calibration_Check_AWScale_plot.py` 离线完成。
#
# 重要前提：
...
EXPERIMENT_TYPE = "AW_ConstOmega_Calibration_Check_AWScale"
RUN_TAG = "const_awscale_omega_check"

# 默认只跑推荐公式以节省时间；legacy_with_mod_offset 保留代码支持但不默认执行。
CONVERSION_MODES = ["linear_voltage"]
# offset=0、2 Vpp 时，任意波归一化值 [-1, +1] 对应等效 DC [-1, +1] V。
# 当前 A_ENV_K/B 下 20000/25000 Hz 会超过 +1 V，默认不扫，避免底层波形被裁剪。
# OMEGA_CONST_LIST_Hz = [5000.0, 10000.0, 15000.0, 20000.0, 25000.0]
OMEGA_CONST_LIST_Hz = [5000.0, 10000.0, 15000.0]
...
def awscale_level_from_voltage(voltage_v, channel):
    """把目标等效 DC 电压换成 offset=0、固定 Vpp 下的 AW 归一化常数值。"""
    half_vpp = 0.5 * float(CONST_AW_VPP)
    if half_vpp <= 0:
        raise ValueError("CONST_AW_VPP 必须为正数")
    level = (float(voltage_v) - float(CONST_AW_OFFSET_V)) / half_vpp
    if level < -1.0 or level > 1.0:
        raise ValueError(
            f"[安全拦截] {channel.upper()} 通道目标等效电压 {float(voltage_v):+.6f} V "
            f"需要 AW normalized={level:+.6f}，超出 [-1, +1]；"
            f"当前 offset={CONST_AW_OFFSET_V} V, amplitude={CONST_AW_VPP} Vpp 只能表达 "
            f"[{CONST_AW_OFFSET_V - half_vpp:+.3f}, {CONST_AW_OFFSET_V + half_vpp:+.3f}] V"
        )
    return float(level)


def validate_awscale_targets():
    """在连接仪器前检查目标频点是否能用固定 2 Vpp AW 表达。"""
    records = []
    for mode in CONVERSION_MODES:
        for omega_hz in OMEGA_CONST_LIST_Hz:
            vx = omega_to_am_voltage(omega_hz, mode, "x")
            vy = omega_to_am_voltage(omega_hz, mode, "y")
            validate_safety_limit("X_magnetic_field_AM", float(vx))
            validate_safety_limit("Y_magnetic_field_AM", float(vy))
            records.append((
                mode,
                float(omega_hz),
                float(vx),
                float(vy),
                awscale_level_from_voltage(vx, "x"),
                awscale_level_from_voltage(vy, "y"),
            ))
    return records
...
def configure_const_aw(dg_am, vx, vy):
    """把 dg_am CH1/CH2 配成 offset=0、2 Vpp 的常数 AW 等效 DC 输出。"""
    validate_voltage_span("X_magnetic_field_AM", CONST_AW_OFFSET_V, CONST_AW_VPP)
    validate_voltage_span("Y_magnetic_field_AM", CONST_AW_OFFSET_V, CONST_AW_VPP)
    validate_safety_limit("X_magnetic_field_AM", float(vx))
    validate_safety_limit("Y_magnetic_field_AM", float(vy))

    x_level = awscale_level_from_voltage(vx, "x")
    y_level = awscale_level_from_voltage(vy, "y")
    x_wave = np.full(CONST_AW_POINTS, x_level, dtype=float)
    y_wave = np.full(CONST_AW_POINTS, y_level, dtype=float)
    for ch in (1, 2):
        dg_am.set_burst_state(False, channel=ch)
        dg_am.set_mod_state(False, channel=ch)
    dg_am.setup_arbitrary(x_wave, freq=A_ENV_FREQ, amplitude=CONST_AW_VPP, offset=CONST_AW_OFFSET_V, channel=1)
    dg_am.setup_arbitrary(y_wave, freq=A_ENV_FREQ, amplitude=CONST_AW_VPP, offset=CONST_AW_OFFSET_V, channel=2)
...
awscale_target_records = validate_awscale_targets()
print("AWScale 目标预检查完成:")
for mode, omega_hz, vx, vy, x_level, y_level in awscale_target_records:
    print(
        f"  {mode}: Omega={omega_hz:.1f} Hz -> "
        f"Vx={vx:+.5f} V (AW={x_level:+.5f}), "
        f"Vy={vy:+.5f} V (AW={y_level:+.5f})"
    )
...
experiment_config = {
    ...
    "run_tag": RUN_TAG,
    "timestamp": timestamp,
    "purpose": "Check constant Omega_ctrl calibration with fixed AW offset=0, amplitude=2 Vpp, and waveform level as equivalent DC.",
    "notes": [
        "Main field, laser powers, and cell temperature are assumed to be already prepared.",
        "Acquisition saves raw traces only; run AW_ConstOmega_Calibration_Check_AWScale_plot.py for peak and residual analysis.",
    ],
    "parameters": {
        ...
        "CONST_AW_OFFSET_V": CONST_AW_OFFSET_V,
        "AW_OUTPUT_METHOD": "fixed_offset0_amplitude2vpp_waveform_level",
...
try:
    for mode in CONVERSION_MODES:
        for omega_hz in tqdm(OMEGA_CONST_LIST_Hz, desc=f"mode={mode}"):
            vx = omega_to_am_voltage(omega_hz, mode, "x")
            vy = omega_to_am_voltage(omega_hz, mode, "y")
            x_aw_level, y_aw_level = configure_const_aw(dg_am, vx, vy)
            ...
            np.savez(
                ...
                const_aw_offset_V=np.float64(CONST_AW_OFFSET_V),
                aw_normalized_x=np.float64(x_aw_level),
                aw_normalized_y=np.float64(y_aw_level),
            ...
            raw_index_records.append({
                ...
                "aw_normalized_x": float(x_aw_level),
                "aw_normalized_y": float(y_aw_level),
            ...
            print(
                ...
                f"Vx={vx:+.5f} V (AW={x_aw_level:+.5f}), Vy={vy:+.5f} V (AW={y_aw_level:+.5f})"
            )
...
print(f"请运行 experiments/AW_ConstOmega_Calibration_Check_AWScale_plot.py 分析: {run_dir}")
