# %% [markdown] Cell 0
# # Bell-Bloom 周期泵浦 R_pump_eff 等效泵浦速率 — duty cycle 扫描
#
# **原理**：利用 Bell-Bloom 磁力仪测量周期泵浦条件下的等效泵浦速率 R_pump_eff。
# 在固定 Pump 光功率 DC 电压下，扫描 pump 脉冲 duty cycle，使用 DG4000 脉冲串 (BURSt) 模式
# 输出 N 个周期的 Bell-Bloom pump 脉冲波形，然后回到 idle (pump off)。
...
# - Pump 光功率 (固定)：DG900 (DG9Q280100002)
#   - CH1 (`Pump_laser_power`): DC FIXED_PUMP_POWER_VOLTAGE — **本实验固定，不扫描**
# - Pump 调制 (duty cycle 扫描变量)：DG4000 (DG4E222800868)
#   - CH1: 100 MHz CW → RF 开关 IN (AOM 载波)
#   - CH2: 90 kHz 脉冲 (FIXED TTL high 5V / low 0V) Burst N cycles
#          → RF 开关 CTRL → AOM → 周期启亮 pump 光
#          **duty cycle 由 DUTY_CYCLE_LIST 逐点扫描**
#   - CH2 同步输出 → 示波器触发通道
...
#
# **扫描**：DUTY_CYCLE_LIST 中每个 duty → 一次 shot → npz
...
def to_builtin_for_yaml(value):
    """将 NumPy 类型转换为 YAML 安全的 Python 内置类型。"""
    if isinstance(value, np.ndarray):
        return value.tolist()
    if isinstance(value, np.generic):
        return value.item()
    if isinstance(value, dict):
        return {key: to_builtin_for_yaml(item) for key, item in value.items()}
    if isinstance(value, (list, tuple)):
        return [to_builtin_for_yaml(item) for item in value]
    return value
...
# ========== 扫描变量: Duty Cycle ==========
# DUTY_CYCLE_LIST 是 DG4000 CH2 PULSe 的 duty cycle 列表 (0~1 小数)
# 每个值写入 set_pulse_dcycle() 前乘以 100 转为百分比
# ⚠ safety_limits.yaml 中 PUMP_MOD_DUTY 限值 1-50%（避免 AOM 过热）
DUTY_CYCLE_LIST = np.linspace(0.02, 0.2, 10)

# ========== Pump 光功率 DC 电压 (固定) ==========
# FIXED_PUMP_POWER_VOLTAGE 是 DG900 CH1 (`Pump_laser_power`) 的 DC 输出，本实验固定不扫描
FIXED_PUMP_POWER_VOLTAGE = 0.2            # Pump 激光 DC (V), 0-1 V 范围
...
# [经验] 5000 cycles @ 90 kHz ≈ 55.6 ms burst; 调整 BURST_CYCLES 确保 tau_BB (几 ms) 量级匹配

# ========== 重复采集 ==========
ACQ_REPEATS = 10                            # 每个 duty 点采集次数, PC 端 mean() 平均
...
# pump 强度由 DG900 CH1 (Pump_laser_power) 决定（本实验固定）；DG4000 CH2 只决定 on/off 时序
# duty cycle 扫描通过 set_pulse_dcycle() 改变每个 burst 周期内的高电平占比
...
validate_safety_limit("Pump_laser_power", FIXED_PUMP_POWER_VOLTAGE)
validate_safety_limit("Time_sequence", PUMP_RF_GATE_HIGH_V)
validate_safety_limit("Time_sequence", PUMP_RF_GATE_LOW_V)
for _d in DUTY_CYCLE_LIST:
    validate_safety_limit("PUMP_MOD_DUTY", _d * 100.0)
...
print(f"Duty cycle 扫描列表 (小数): {DUTY_CYCLE_LIST}")
print(f"Pump DC (固定): {FIXED_PUMP_POWER_VOLTAGE} V")
...
try:
    ...
    dg_laser.setup_dc(FIXED_PUMP_POWER_VOLTAGE, channel=pump_cfg["channel"])
    dg_laser.setup_dc(PROBE_POWER, channel=probe_cfg["channel"])
    print(f"  Pump DC: {FIXED_PUMP_POWER_VOLTAGE} V (CH{pump_cfg['channel']}, "
          f"固定), "
...
# ---- 4. DG4000 CH2: 周期泵浦 Burst 门控 (TTL high/low 固定，duty cycle 为扫描变量) ----
# CH2 输出 90 kHz 脉冲 (5V TTL 高/0V TTL 低) → RF 开关 CTRL → AOM → 周期启亮 pump 光
# [经验] RF 开关 CTRL 是数字门控，high/low 都是固定的 (TTL 5V/0V)；
#       真正控制 pump 强度的是 DG900 CH1 (`Pump_laser_power`)，本实验固定不扫描
#       duty cycle 通过 set_pulse_dcycle() 改变，控制每个 BB 周期内 pump on 占比
...
dg_rf.set_pulse_dcycle(DUTY_CYCLE_LIST[0] * 100.0, channel=2)  # 初始值
# 固定 TTL 电平: 高 5V, 低 0V (数字门控，不是 pump 强度)
...
print(f"CH2: {MODULATION_FREQUENCY_HZ/1000:.0f} kHz 脉冲, "
      f"占空比 (初始) {DUTY_CYCLE_LIST[0]*100}%, "
...
print(f"  Duty cycle 扫描: 由 DUTY_CYCLE_LIST 逐点写入 set_pulse_dcycle()")
...
def build_scope_config(pd_scale, sync_scale=SDS_VOLT_SCALE_TRIG):
    """构建 SDS 采集配置：CH1=PD (BB 信号), CH4=DG4000 同步输出。"""
    channels = []
...
config_snapshot = {
    ...
    "fixed_pump": {
        "fixed_pump_power_voltage": FIXED_PUMP_POWER_VOLTAGE,
        "control_channel": f"DG900 CH{pump_cfg['channel']} (`Pump_laser_power` 通道, DC 输出)",
        "rf_gate_role": "DG4000 CH2 (TTL high/low 固定, duty cycle 扫描)",
    },
    "main_field": {
    ...
    "probe": {
        ...
        "control_channel": f"DG900 CH{probe_cfg['channel']} (`Probe_laser_power` 通道, 固定 DC)",
    },
    "aom_carrier": {
...
with open(config_path, "w", encoding="utf-8") as f:
    yaml.safe_dump(
        to_builtin_for_yaml(config_snapshot),
        f,
...
def acquire_single_shot(duty_cycle, current_scale, scope_cfg_ref):
    """执行单次 burst pump shot 采集。

    温控开关策略：
      - 采集开始前: Temp_Switch = 0V, 等 0.1s 让温控磁场衰减, 避免干扰 Bell-Bloom 信号
      - 采集结束后: Temp_Switch = 5V, 等 2.0s 让温控回路重新稳定 (异常退出也会恢复)

    Parameters
    ----------
    duty_cycle : float
        本次 shot 的 duty cycle (0~1 小数), 写入 DG4000 CH2 set_pulse_dcycle()。
        DG4000 CH2 的 TTL 高/低电平在 Cell 4 已固定, 不修改。
        DG900 CH1 (Pump_laser_power) DC 电压在 Cell 4 已固定, 不修改。

    Returns
    -------
    dict with keys:
        time_s, signal_v, duty_cycle, scale_used, t_burst_start_s, t_burst_end_s
    """
    # 1. 更新 DG4000 CH2 pulse duty cycle (扫描变量)
    # [经验] 必须先关闭 burst 再修改 duty，否则 SCPI 报错
    dg_rf.set_burst_state(False, channel=2)
    dg_rf.set_pulse_dcycle(duty_cycle * 100.0, channel=2)
    dg_rf.set_burst_state(True, channel=2)  # 重新开启 burst mode
    time.sleep(0.05)
    ...
    try:
        ...
        if not sds_inst.wait_for_trigger(timeout=BURST_BURST_PERIOD_S + 5.0):
            print(f"    ⚠ Duty cycle={duty_cycle*100:.1f}% 触发超时")
...
# %% Cell 5b
# ============================================================
# 主采集循环：扫描 DUTY_CYCLE_LIST → DG4000 CH2 set_pulse_dcycle()
# ============================================================
print("=" * 60)
print(f"周期泵浦 R_pump_eff duty cycle 扫描: {len(DUTY_CYCLE_LIST)} 点, "
      f"每点重复 {ACQ_REPEATS} 次 PC 端平均")
print(f"  频率: {MODULATION_FREQUENCY_HZ/1000:.0f} kHz, "
      f"Burst: {BURST_CYCLES} cycles ({BURST_DURATION_S*1000:.2f} ms)")
print(f"  RF gate (DG4000 CH2, 固定): high={PUMP_RF_GATE_HIGH_V}V / low={PUMP_RF_GATE_LOW_V}V TTL")
print(f"  Pump DC (DG900 CH1, 固定): {FIXED_PUMP_POWER_VOLTAGE} V")
print(f"  Duty cycle 扫描: {DUTY_CYCLE_LIST}")
print("=" * 60)

all_shots = []
try:
    for idx, duty_cycle in enumerate(tqdm(DUTY_CYCLE_LIST, desc="Duty 扫描")):
        print(f"\n--- [{idx+1}/{len(DUTY_CYCLE_LIST)}] "
              f"Duty cycle = {duty_cycle*100:.1f}% ---")
        ...
        if probe_shot is None:
            print(f"  ⚠ Duty cycle={duty_cycle*100:.1f}% 采集失败，跳过")
            continue
        ...
        for rep, s in enumerate(locked_shots):
            np.savez(
                raw_dir / f"shot_d{idx:02d}_r{rep}.npz",
                time_s=s["time_s"],
                signal=s["signal_v"],
                duty_cycle=duty_cycle,
                rep_index=rep,
        ...
        # 平均 npz (供 plot 拟合用)
        out_file = raw_dir / f"shot_d{idx:02d}_avg.npz"
        np.savez(
            out_file,
            time_s=time_s_avg,                  # ⚠ 权威时间参考
            signal=signal_avg,                 # ACQ_REPEATS 次平均
            duty_cycle=duty_cycle,
            pump_power_voltage=FIXED_PUMP_POWER_VOLTAGE,
    ...
    # [安全] 异常时关闭 burst, DG4000 CH2 回到 idle DC
    try:
        dg_rf.set_burst_state(False, channel=2)
        dg_rf.set_output(False, channel=2)
        dg_rf.set_sync_state(False, channel=2)
        print(f"\n[安全] Burst OFF, Pump DC 保持 {FIXED_PUMP_POWER_VOLTAGE}V, probe 不变")
    except Exception as e:
        print(f"[安全] 恢复失败: {e}")

# ---- 保存汇总数据 (供 plot 脚本读取) ----
np.savez(
    raw_dir / "duty_scan_summary.npz",
    duty_cycle=[s["duty_cycle"] for s in all_shots],
    files=[s["file"] for s in all_shots],
    fixed_pump_power_voltage=FIXED_PUMP_POWER_VOLTAGE,
    n_shots=len(all_shots),
)
print(f"\n汇总已保存: {raw_dir / 'duty_scan_summary.npz'} ({len(all_shots)} shots)")

print("\n✅ 周期泵浦 R_pump_eff duty cycle 扫描完成")
...
if tec and hasattr(tec, "disconnect"):
    try:
        ...
        print(f"  TEC 断开失败: {e}")
print("其他设备保持连接，Burst OFF, Pump_laser_power 保持原值")
