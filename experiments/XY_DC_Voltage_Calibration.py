# %% [markdown] Cell 0
# # XY DC 电压标定 — ConstXY 链路
#
# 在当前 ConstXY 实验链路下，重新标定 X/Y AM 控制 DC 电压
...
#   `X_magnetic_field_AM` / `Y_magnetic_field_AM` 的 AM DC 包络实现。
...
#
# ## 输出
# - 目录: `data/XY_DC_Voltage_Calibration/<MMDD_HHMM_xy_dc_cal>/`
...
# ========== 校相时的 XY DC 电压与状态保持 ==========
# 本实验在校相时必须保持 ConstXY 链路开启（carrier ON + dg_am 输出 DC 包络），
...
# 分通道相位校准。通过 dg_am 输出 AM DC 包络 + dg_comp 90 kHz AM EXT 载波，
...
XY_PHASE_CAL_AM_DC_V = float(PHASE_CAL_XY_DC_VOLTAGE_V)  # 校相时 dg_am DC 包络
...
for am_key in ("X_magnetic_field_AM", "Y_magnetic_field_AM"):
    if am_key not in LIMITS:
        ...
        print(f"       description: \"DG4E231500376 CH1/CH2 X/Y AM DC 包络\"")
    else:
        lim = LIMITS[am_key]
...
print(f"XY_DC 电压扫描: {len(XY_DC_VOLTAGE_LIST)} 点 "
...
#       DC 包络 = XY_DC_VOLTAGE_V，由 dg_am CH1/CH2 输出至 dg_comp AM 输入。
#
...
def set_xy_am_dc(dg_am_inst, v_dc, output=True):
    """设置 X/Y AM DC 包络电压，并对 X/Y AM 物理量做安全检查.

    顺序：
      1) burst OFF、mod OFF（复位残留调制模式）
      2) setup_dc(v_dc, channel=1/2)
      3) set_output(output, channel=1/2)

    Args:
        dg_am_inst: DG4000Instrument 实例（dg_am）
        v_dc:        DC 包络电压 (V)
        output:      True=输出 ON，False=输出 OFF

    Returns:
        float: 实际设置的 v_dc（float 化）。
    """
    v_dc = float(v_dc)

    # 安全校验：如果 safety_limits.yaml 没定义 *_AM 限值，
    # validate_safety_limit 不抛错（沿用启动时的 WARNING）。
    validate_safety_limit("X_magnetic_field_AM", v_dc)
    validate_safety_limit("Y_magnetic_field_AM", v_dc)
    ...
    print(f"  dg_am CH1/CH2: DC={v_dc:.4f} V, output={state} "
          f"(→ dg_comp AM EXT 包络)")
    return v_dc
...
def shutdown_xy_chain(dg_comp_inst, dg_am_inst):
    ...
    关闭 dg_am CH1/CH2 (AM DC 包络) 输出，但不修改它们的波形/参数。
    """
    # 关 dg_am
    try:
        for ch in (1, 2):
...
def set_xy_phase_cal_am_dc(dg_am_inst, v_dc):
    ...
    顺序：burst OFF、mod OFF → setup_dc(v_dc, ch=1/2)。
    不主动 set_output；由 calibrate_xy_carrier_phase() 按需控制。
    """
    v_dc = float(v_dc)
    validate_safety_limit("X_magnetic_field_AM", v_dc)
    validate_safety_limit("Y_magnetic_field_AM", v_dc)
...
# dg_am (DC 包络)。在 Phase 1 校相时，ConstXY 链路必须 OFF（基线纯净）；
...
if ENABLE_XY_CARRIER_PHASE_CAL:
    ...
    #   3. set_xy_phase_cal_am_dc (AM DC 包络)
...
config = {
    ...
    "purpose": (
        "在当前 ConstXY 链路下，重新标定 X/Y AM DC 电压 XY_DC_VOLTAGE 与"
    ...
    "phase_calibration": {
        ...
        "note": (
            ...
            "把 dg_am CH1/CH2 输出 PHASE_CAL_XY_DC_VOLTAGE_V DC 包络。"
    ...
    "signal_chain_note": (
        ...
        "CH1/CH2) 的 AM DC 包络，由 dg_comp AM 输入端接收。真正产生 XY 旋转场的是 "
        "dg_comp 载波被 dg_am DC 包络调幅后的输出。"
    ),
    "pump_modulation": {
...
print("=" * 60)
print(f"XY 电压点数: {len(XY_DC_VOLTAGE_LIST)}")
...
try:
    ...
    # 每个 xy_dc_v 点只切换 dg_am 的 DC 包络，不重配 dg_comp。
    ...
    for v_idx, xy_dc_v in enumerate(tqdm(XY_DC_VOLTAGE_LIST,
        ...
        # ---- 切换 XY DC 电压（只改 dg_am DC 包络，carrier 保持 ON）----
        set_xy_am_dc(dg_am, float(xy_dc_v), output=True)
        time.sleep(XY_DC_SETTLE_TIME_s)
        print(f"\n[XY DC] v_idx={v_idx:02d}, XY_DC_VOLTAGE={xy_dc_v:.4f} V "
