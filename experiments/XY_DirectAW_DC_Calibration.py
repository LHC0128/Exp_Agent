...
CLOCK_PROFILE = load_clock_profile(project_root / 'params' / 'clock_sources.yaml')
...
# 每次重新上传 DirectAW 前停止触发，等待触发线稳定后再共同重新触发 X/Y。
XY_TRIGGER_REARM_GUARD_s = 0.05
...
if not np.isclose(PUMP_MOD_FREQ_Hz, XY_CARRIER_FREQ_Hz):
    raise ValueError("DirectAW 载波频率必须与 Pump 调制频率一致")
...
if XY_AW_POINTS > DG4000Instrument.MAX_ARB_POINTS:
    raise ValueError("XY_AW_POINTS 超出 DG4000 单波形点数上限")
...
def normalize_clock_source(value):
    '''把仪器时钟源读回值统一为 clock_sources.yaml 使用的缩写。'''
    return 'EXT' if str(value).upper().startswith('EXT') else 'INT'


def resource_device_id(resource):
    '''从 VISA resource 中提取设备序列号。'''
...
def verify_dg_clock(label, cfg, inst, required=True):
    '''验证 DG 时钟源；DC-only 设备可只记录警告而不中止实验。'''
    device_id = resource_device_id(cfg['resource'])
    target = CLOCK_PROFILE.get(device_id, CLOCK_PROFILE['__default__'])
    actual = normalize_clock_source(inst.get_ref_clock_source())
    record = {'device_id': device_id, 'target': target, 'actual': actual}
    clock_verification[label] = record
    if actual != target:
        message = (
            f'{label} ({device_id}) 时钟源不一致: 期望 {target}, 实际 {actual}。'
        )
        if required:
            raise RuntimeError(
                message
                + '请先运行 experiments/Set_All_Clocks_External.py；'
                + '标定脚本不会修改时钟源。'
            )
        record['warning'] = message + '该设备仅输出 DC，不影响本实验时序，继续运行。'
        print(f"  ⚠ {record['warning']}")
        return
    print(f'  {label} 时钟验证通过: {actual}')


def verify_hf2_clock(label, cfg, inst):
    '''只验证 HF2 时钟源，不修改。'''
    device_id = cfg['device_id']
    target = CLOCK_PROFILE.get(device_id, CLOCK_PROFILE['__default__'])
    actual = 'EXT' if inst.get_extclk() else 'INT'
    record = {'device_id': device_id, 'target': target, 'actual': actual}
    clock_verification[label] = record
    if actual != target:
        raise RuntimeError(
...
try:
    ...
    dg_comp = DG4000Instrument(comp_cfg["resource"], channel=1)
    dg_comp.connect()
    devices["dg_comp"] = dg_comp
    verify_dg_clock('dg_comp', comp_cfg, dg_comp)
    reset_dg_channels(dg_comp)

    trigger_cfg = MAPPING["X_magnetic_field_AM"]
    dg_trigger = DG4000Instrument(trigger_cfg["resource"], channel=1)
    dg_trigger.connect()
    devices["dg_trigger"] = dg_trigger
    verify_dg_clock('dg_trigger', trigger_cfg, dg_trigger)
    reset_dg_channels(dg_trigger)

    sweep_cfg = MAPPING["Z_magnetic_field"]
    dg_sweep = DG4000Instrument(sweep_cfg["resource"], channel=1)
    dg_sweep.connect()
    devices["dg_sweep"] = dg_sweep
    verify_dg_clock('dg_sweep', sweep_cfg, dg_sweep)
    reset_dg_channels(dg_sweep)

    mod_cfg = MAPPING["Pump_modulation"]
    dg_mod = DG4000Instrument(mod_cfg["resource"], channel=1)
    dg_mod.connect()
    devices["dg_mod"] = dg_mod
    verify_dg_clock('dg_mod', mod_cfg, dg_mod)
    reset_dg_channels(dg_mod)

    laser_cfg = MAPPING["Pump_laser_power"]
    dg_laser = DG900Instrument(laser_cfg["resource"], channel=1)
    dg_laser.connect()
    devices["dg_laser"] = dg_laser
    verify_dg_clock('dg_laser', laser_cfg, dg_laser, required=False)

    temp_cfg = MAPPING["Temp_Switch"]
    dg_temp = DG900Instrument(temp_cfg["resource"], channel=2)
    dg_temp.connect()
    devices["dg_temp"] = dg_temp
    verify_dg_clock('dg_temp', temp_cfg, dg_temp, required=False)
    ...
    hfi.connect()
    devices["hf2"] = hfi
    verify_hf2_clock('hf2', hf2_cfg, hfi)
except Exception:
...
def configure_trigger_source(output=True):
    """配置 dg_trigger 双通道同相方波，触发 dg_comp DirectAW。"""
    ...
    for ch in (1, 2):
        dg_trigger.set_output(bool(output), channel=ch)
    if output:
        for ch in (1, 2):
...
def upload_direct_aw(envelope_v, phase_deg, output=True):
    """上传两路 DirectAW，并在两路均就绪后重新启动公共相位触发。"""
    wave = build_direct_aw(envelope_v, phase_deg)

    # 上传期间禁止新触发沿，避免 CH1 已运行而 CH2 尚未完成配置。
    for ch in (1, 2):
        dg_trigger.set_output(False, channel=ch)
    time.sleep(XY_TRIGGER_REARM_GUARD_s)
    ...
    if output:
        # 两路都上传并配置完成后再共同进入外触发等待状态。
        for ch in (1, 2):
            dg_comp.set_output(True, channel=ch)

        # 重新启动 dg_trigger，使 X/Y 从同一轮正沿开始运行。
        for ch in (1, 2):
            dg_trigger.phase_init(channel=ch)
        for ch in (1, 2):
...
def safe_outputs_off():
    ...
    try:
        ...
        print(f"Z 场关闭失败: {exc}")
    for inst, name in ((dg_comp, "dg_comp"), (dg_trigger, "dg_trigger")):
        try:
            for ch in (1, 2):
                inst.set_burst_state(False, channel=ch)
                inst.set_mod_state(False, channel=ch)
...
calibrated_phase_0_deg = demod.auto_calibrate_phase(
    hfi,
...
# ---- DirectAW 整体相位校准 ----
# 只配置方波，保持输出 OFF；每次两路 DirectAW 上传完成后再重新触发。
configure_trigger_source(output=False)
...
if ENABLE_XY_PHASE_CAL:
    ...
    try:
        for iteration in range(XY_PHASE_CAL_MAX_ITER + 1):
            upload_direct_aw(XY_PHASE_CAL_ENVELOPE_V, xy_phase_deg, output=True)
            ...
            phase_history.append(
                {
                    ...
                    "xy_phase_deg": float(xy_phase_deg),
                    "theta_deg": float(theta_deg),
                    "r_V": float(sample["r"]),
                }
            )
            print(
                f"DirectAW 校相 {iteration}: phase={xy_phase_deg:.3f}°, "
                f"theta={theta_deg:+.3f}°, R={sample['r']:.4e} V"
            )
            if abs(theta_deg) < XY_PHASE_CAL_TOL_deg:
                break
            xy_phase_deg = (xy_phase_deg - theta_deg) % 360.0
    finally:
        dg_temp.set_output(True, channel=2)
        time.sleep(1.0)
phase_cal_success = bool(
    phase_history and abs(phase_history[-1]["theta_deg"]) < XY_PHASE_CAL_TOL_deg
)
...
config = {
    ...
    "xy_trigger": {
        ...
        "rearm_guard_time_s": XY_TRIGGER_REARM_GUARD_s,
        "restart_policy": (
            "trigger OFF -> dg_comp CH1/CH2 upload and arm -> "
            "trigger phase init and outputs ON"
        ),
    },
    "z_frequency_scan": {
