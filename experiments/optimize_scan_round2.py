"""第二轮精细优化: Pump>0.5V, Probe<0.1V, Duty微调, 组合Pump+Duty."""
...
OPTIMIZE_SEQUENCE = [
    ...
    # 2) 在 Pump=0.5V, Duty=best 基础上, 继续推高 Pump
    {
    ...
    # 4) 组合扫描: Pump + Duty 二维网格 (选最优组合)
    #    Pump in [0.4, 0.5, 0.6], Duty in [10, 15]
    #    作为 combo scan, 我们用固定格式标记
    {
        "parameter": "Combo:Pump_Duty",
        "values": ["0.4_10", "0.4_15", "0.5_10", "0.5_15", "0.6_10", "0.6_15"],
        "stabilize_time": 2.0,
        "recalibrate_phase": True,
    },
]
...
print(f"R2 优化: {len(OPTIMIZE_SEQUENCE)} 个维度, 共 {total_iters} 次迭代")
print(f"基准: {BASELINE}")
...
def apply_param(name, val):
    ...
    if name == "Pump_laser_power":
        dg_laser.setup_dc(float(val), channel=1)
    elif name == "Probe_laser_power":
        dg_laser.setup_dc(float(val), channel=2)
    elif name == "main_magnetic_field":
        gs.set_current(float(val) / 1000)
...
def apply_combo(val_str):
    """应用组合参数 'Pump_Duty'."""
    parts = val_str.split("_")
    pump_v, duty_v = float(parts[0]), int(parts[1])
    apply_param("Pump_laser_power", pump_v)
    apply_param("PUMP_MOD_DUTY", duty_v)
...
try:
    ...
    for seq_idx, seq_cfg in enumerate(OPTIMIZE_SEQUENCE):
        ...
        for i, val in enumerate(values):
            ...
            tag = f"R2_{pname}_{val}".replace(":", "_")
            ...
            if pname.startswith("Combo:"):
                apply_combo(val)
            else:
                apply_param(pname, val)
            time.sleep(stab)

            if pname.startswith("Combo:") or do_recal:
                p = recalibrate()
            ...
            params_dict = get_all_params()
            if pname.startswith("Combo:"):
                parts = val.split("_")
                params_dict["Pump_laser_power"] = float(parts[0])
                params_dict["PUMP_MOD_DUTY"] = int(parts[1])
            else:
                params_dict[pname] = val
            ...
            if valid_res:
                ...
                print(f"  >>> Best: {best['param']}={best['value']}, "
    ...
    valid_res = [r for r in all_results if r["status"] == "OK"]
    print(f"Valid: {len(valid_res)}/{len(all_results)}")

    valid_res.sort(key=lambda r: r["sens"].sens_flat)
    print(f"\n=== All Results (sorted by sensitivity) ===")
    print(f"{'Rank':<5} {'Param':<22} {'Value':<10} {'Sens(fT/sqrtHz)':<16} {'HWHM(nT)':<10} {'Slope(V/fT)':<14} {'R^2':<8}")
    print("-" * 85)
    for rank, r in enumerate(valid_res):
        fr = r["fit"]; sr = r["sens"]
        print(f"{rank+1:<5} {r['param']:<22} {r['value']:<10} "
