# SDS 示波器控制模块 (`sds_acquisition`)

控制 SDS 系列数字示波器，基于 PyVISA + SCPI 协议。

专用 GUI 位于“实验中心 → 示波器采集”，支持单通道采集、软件 PSD 和历史比较，详见 [示波器采集](scope_capture.md)。
...
src/sds_acquisition/
  ...
  config.py          # 配置数据类 (AcquisitionConfig, ChannelConfig, TriggerConfig, AcquisitionResult)
  acquire.py         # 采集编排器 (SDSAcquisition)
  waveform.py        # 波形解析与转换 (WaveformPreamble, ADC 转电压)
  save.py            # 数据保存与加载 (CSV/NPZ/MAT/HDF5/BIN)
  cli.py             # 命令行入口
```
