...
mapping_keys:
  ...
  main_magnetic_field:
    role: gs200_zero_off_physically_disconnected
  Time_sequence_2:
    role: common_trigger
  rf_coil:
...
required_devices:
  ...
  - HF2
learned_notes:
  - 6221 独占 Z 主场线圈，GS200 必须物理断开并由程序保持 0 mA、输出关闭。
  ...
  - 所有退出路径都将 6221 与 GS200 归零并关闭，不恢复运行前非零状态。
---
...
该实验仿照 `mx-z-optimal-control-rf-sensitivity` 的 Y RF 正交校相、带符号幅度扫描、
RF-off 噪声采集和离线分析，但将 Z 主场最优控制从 DG4000 任意波改为 Keithley
6221 内部 ARB0。原实验不修改，两种硬件方案保持独立。
