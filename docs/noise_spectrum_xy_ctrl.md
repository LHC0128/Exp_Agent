...
参考时钟目标以 `params/clock_sources.yaml` 为准：Pump 调制 DG4000 使用内部时钟，
其余已连接 DG4000、两台 DG900 和 HF2 使用外部时钟。工作流统一设置并回读验证，
保证参考时钟配置与仓库声明一致。
...
- [经验] 外部触发同步 + `clock_sources.yaml` 声明的 10 MHz 主从时钟配置保证长期相位稳定
