...
type GeneratorEditorProps = {
  ...
  onSaved: (value: GeneratorSnapshot) => void;
};

type ChannelTab = "wave" | "mod" | "burst";
type NestedSettings = { mod: ModulationSnapshot; burst: BurstSnapshot };

export function GeneratorEditor({ snapshot, onSaved }: GeneratorEditorProps) {
  ...
  const save = async (index: number) => {
    ...
    const settings: GeneratorChannelSettings = {
    ...
    setError("");
    try {
      const saved = await api<GeneratorSnapshot>(
        `/api/devices/${snapshot.id}/channels/${channel.number}`,
        ...
        return (
          <section className="panel generator-panel" key={channel.number}>
            ...
            {tab === "wave" && (
              <div className="form-grid">
                <SelectField label="波形" value={channel.shape} disabled={disabled} onChange={(value) => update(index, "shape", value)}>
                  <option value="SINusoid">Sine</option><option value="SQUare">Square</option>
                  <option value="RAMP">Ramp</option><option value="PULSe">Pulse</option><option value="DC">DC</option>
                </SelectField>
                {!channel.shape.startsWith("DC") && (
                  <>
                    <Field label="频率 (Hz)" value={channel.frequency} disabled={disabled} onChange={(value) => update(index, "frequency", value)} />
                    <Field label="幅度 (Vpp)" value={channel.amplitude} disabled={disabled} onChange={(value) => update(index, "amplitude", value)} />
                  </>
                )}
                <Field label="偏置 (V)" value={channel.offset} disabled={disabled} onChange={(value) => update(index, "offset", value)} />
                {!channel.shape.startsWith("DC") && (
                  <Field label="相位 (deg)" value={channel.phase} disabled={disabled} onChange={(value) => update(index, "phase", value)} />
                )}
                {snapshot.type === "DG4000" && (
                  <>
              ...
              <>
                ...
                <div className="form-grid">
                  <SelectField label="Burst 模式" value={burst.mode || "TRIGgered"} disabled={disabled} onChange={(value) => updateNested(index, "burst", "mode", value)}>
                    <option value="TRIGgered">Triggered</option><option value="GATed">Gated</option>{snapshot.type === "DG4000" && <option value="INFinity">Infinity</option>}
                  </SelectField>
                  {burst.mode !== "GATed" && (
                    <>
            ...
            <div className="panel-actions">
              <button disabled={disabled || saving === channel.number} onClick={() => save(index)}>
