...
``CommonMxZControlParams`` 集中工作点、RF、噪声、相位和触发参数，
不包含控制波形来源选择；``MxZControlSourceSelectionParams`` 在其上
保留理论换算与闭环冻结波形的双来源字段、来源校验和历史迁移，
供仍需两种来源的实验继承。
"""
...
class CommonMxZControlParams(MxYRFParams):
    """Mx Z 周期控制实验的公共工作点参数。"""
    ...
    # 以下三个默认值覆盖共享基类，与本系实验的版本化 YAML 保持一致。
    r_bad_point_std_threshold_v: float = parameter(default=0.1, external_name="R_BAD_POINT_STD_THRESHOLD_V", label="R 坏点标准差阈值", unit="V", group="advanced", minimum=0)
    temp_switch_on_lag_s: float = parameter(default=2.0, external_name="TEMP_SWITCH_ON_LAG_S", label="温控恢复后等待", unit="s", group="advanced", minimum=0)
    fit_r_squared_min: float = parameter(default=0.1, external_name="FIT_R_SQUARED_MIN", label="拟合最低 R²", group="advanced", minimum=0, maximum=1)
...
@dataclass(slots=True)
class MxZControlSourceSelectionParams(CommonMxZControlParams):
    """保留理论换算与闭环冻结波形双来源的兼容参数模型。"""
    ...
    def migrate_external(
        ...
        migrated = super(MxZControlSourceSelectionParams, cls).migrate_external(
            values,
            schema_version,
        )
        if schema_version < 3:
