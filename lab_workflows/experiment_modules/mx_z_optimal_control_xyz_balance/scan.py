...
def build_xyz_axes(
    ...
    return (
        ...
        _build_axis(
            params.z_field_start_ma,
            params.z_field_stop_ma,
            params.z_field_points,
            "Z",
        ),
    )
...
def iter_serpentine_grid(
    ...
    """按 Z 外层连续蛇形返回采集序号和规范 Z/X/Y 索引。"""
    x_axis, y_axis, z_axis = build_xyz_axes(params)
    acquisition_index = 0
    row_index = 0
    for z_index, z_ma in enumerate(z_axis):
        x_indices = (
            range(x_axis.size)
            if z_index % 2 == 0
