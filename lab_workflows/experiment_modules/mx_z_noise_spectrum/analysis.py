...
def _load_params(
    run_dir: Path,
) -> tuple[MxZNoiseSpectrumParams, dict[str, Any]]:
    config_path = run_dir / "experiment_config.yaml"
    if not config_path.exists():
        raise FileNotFoundError(f"未找到实验配置: {config_path}")
    with config_path.open(encoding="utf-8") as stream:
        config = yaml.safe_load(stream) or {}
    params = MxZNoiseSpectrumParams.from_external(
...
def _load_waveforms(
    ...
    for path in sorted(raw_dir.glob("waveform_Z*.npz")):
        with np.load(path) as data:
            records.append(
                {
                    "path": path,
                    "point_index": int(data["point_index"]),
                    "target_detuning_hz": float(data["target_detuning_hz"]),
                    "absolute_resonance_frequency_hz": float(
                        data["absolute_resonance_frequency_hz"]
                    ),
                    "z_voltage_v": float(data["z_voltage_v"]),
    ...
    if len(records) < 10:
        raise ValueError(
            f"至少需要 10 个完整 Z 噪声点，当前仅发现 {len(records)} 个"
        )
...
def _compute_psd_matrix(
    ...
    for record in records:
        ...
        if frequency_axis is None:
            ...
            raise ValueError("各 Z 点的实际采样率或 Welch 频率轴不一致")
        rows.append(psd)
...
def _plot_control_axis(
    results_dir: Path,
    z_voltage_v: np.ndarray,
    detuning_hz: np.ndarray,
) -> str:
    filename = "z_control_axis.png"
    figure, axis = new_figure()
    axis.plot(z_voltage_v, detuning_hz / 1000.0, "o-", color=COLOR_OPTIMAL)
    format_axis(
        axis,
        xlabel="Z DC voltage (V)",
        ylabel="Control detuning (kHz)",
    )
...
def analyze(run_dir: Path) -> dict[str, Any]:
    ...
    target_detuning_hz = np.asarray(
        [record["target_detuning_hz"] for record in records], dtype=float
    )
    z_voltage_v = np.asarray(
        [record["z_voltage_v"] for record in records], dtype=float
    )
    absolute_resonance_hz = np.asarray(
        [record["absolute_resonance_frequency_hz"] for record in records],
        dtype=float,
    )
    actual_rate_sa_s = np.asarray(
        [record["actual_rate_sa_s"] for record in records], dtype=float
    )
    calibrated_detuning_hz = (
        params.z_calibration_hz_per_v * z_voltage_v
    ...
    if not np.allclose(
        ...
        raise ValueError("原始数据中的 Z 电压不满足保存的严格标定式")
    ...
    np.savez(
        ...
        target_detuning_hz=target_detuning_hz,
        calibrated_detuning_hz=calibrated_detuning_hz,
        absolute_resonance_frequency_hz=absolute_resonance_hz,
    ...
    np.savez(
        ...
        omega_ctrl=calibrated_detuning_hz,
        target_detuning_hz=target_detuning_hz,
        z_voltage_v=z_voltage_v,
        fit_mask=fit.fit_mask,
        interpolated_mask=fit.interpolated_mask,
    ...
    files = [
        ...
        "noise_spectra.npz",
        "noise_spectra.csv",
        _plot_control_axis(results_dir, z_voltage_v, calibrated_detuning_hz),
        _plot_psd_matrix(
            results_dir,
            frequency_axis_hz,
            calibrated_detuning_hz,
    ...
    payload = {
        ...
        "experiment_id": config.get("experiment_id", "mx-z-noise-spectrum"),
        "run_dir": str(run_dir),
        "acquisition_signal": "Demod0 R",
        "control_model": (
            "detuning_Hz = K_Z_Hz_per_V * Z_voltage_V "
            "+ f_0V_Hz - HF2_oscillator_Hz"
        ),
        "z_calibration": {
            "source_run": params.z_calibration_source_run,
            "slope_hz_per_v": params.z_calibration_hz_per_v,
            "intercept_hz": params.z_calibration_intercept_hz,
            "hf2_oscillator_hz": params.zero_bias_reference_frequency_hz,
        },
        "scan": {
            "completed_points": len(records),
            "target_points": params.target_detuning_points,
            "detuning_start_hz": float(calibrated_detuning_hz[0]),
            "detuning_stop_hz": float(calibrated_detuning_hz[-1]),
            "z_voltage_start_v": float(z_voltage_v[0]),
            "z_voltage_stop_v": float(z_voltage_v[-1]),
        },
        "welch": {
    ...
    print(
        f"Mx Z 噪声谱分析完成: {len(records)} 个 Z 点，"
