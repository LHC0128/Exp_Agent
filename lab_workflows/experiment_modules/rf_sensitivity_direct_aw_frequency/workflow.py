...
PARAMS = load_runtime_params(RFDirectAWFrequencyParams)
...
if Z_RF_FREQ_LOG_SPACED:
    ...
    freq_list = (
        np.linspace(Z_RF_FREQ_START, Z_RF_FREQ_STOP, Z_RF_FREQ_POINTS)
        + Z_RF_FREQ_OFFSET_HZ
    )

has_baseline = bool(np.any(np.isclose(freq_list, 0.0)))
...
phase_cal_success = bool(
    direct_aw_phase_result.converged
    if ENABLE_XY_CTRL_PHASE_CAL
    else abs(theta_now) < XY_CTRL_PHASE_TOL_DEG
)
print(
...
calib_results = {
    ...
    "best_r_V": (
        float(direct_aw_phase_result.best_r_v)
        if ENABLE_XY_CTRL_PHASE_CAL
        else float(sample_now["r"])
    ),
...
def safe_outputs_off():
    ...
    return run_safety_shutdown(
        dg_channels=(
            DGChannelShutdown(dg_sweep, 1, "Z_magnetic_field", "Z 射频场"),
...
try:
    ...
    if shutdown_report.errors:
        ...
        print("安全关闭完成，温度开关已恢复 ON，TEC 已断开")
