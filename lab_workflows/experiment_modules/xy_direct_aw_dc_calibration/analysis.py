...
# 当响应最大采样点进入扫频边界一个步长以内时，说明峰形没有被完整覆盖。
# 一旦某个 |V| 首次触及上边界，更高 |V| 的真峰只会继续移出扫描范围，
# 即使程序在较低频率误抓到次峰，也不得用于电压标定。
SWEEP_EDGE_MARGIN_STEPS = 1.0
...
def sweep_boundary_status(freqs, fit, margin_steps=SWEEP_EDGE_MARGIN_STEPS):
    """判断拟合峰是否已触及扫频边界，避免把边界或次峰用于标定。"""
    freqs = np.asarray(freqs, dtype=float)
    finite_freqs = np.sort(np.unique(freqs[np.isfinite(freqs)]))
    if len(finite_freqs) == 0:
        return {
            ...
            "upper_limited": True,
            "margin_Hz": np.nan,
            "scan_range_Hz": [np.nan, np.nan],
        }

    positive_steps = np.diff(finite_freqs)
    positive_steps = positive_steps[positive_steps > 0]
    step_Hz = float(np.median(positive_steps)) if len(positive_steps) else 0.0
    margin_Hz = max(float(margin_steps) * step_Hz, np.finfo(float).eps)
    lower_Hz = float(finite_freqs[0])
    upper_Hz = float(finite_freqs[-1])
    peak_locations = np.asarray(
        [fit.get("center_Hz", np.nan), fit.get("max_bin_Hz", np.nan)],
        dtype=float,
    )
    peak_locations = peak_locations[np.isfinite(peak_locations)]
    lower_limited = bool(
        len(peak_locations) == 0 or np.any(peak_locations <= lower_Hz + margin_Hz)
    )
    upper_limited = bool(
        len(peak_locations) == 0 or np.any(peak_locations >= upper_Hz - margin_Hz)
    )
    return {
        "limited": lower_limited or upper_limited,
        "lower_limited": lower_limited,
        "upper_limited": upper_limited,
        "margin_Hz": margin_Hz,
        "scan_range_Hz": [lower_Hz, upper_Hz],
    }
...
for envelope_v in envelope_values:
    ...
    boundary_status = sweep_boundary_status(z_freqs, primary_fit)
    per_voltage[float(envelope_v)] = {
        ...
        "sweep_boundary_status": boundary_status,
    ...
    print(
        ...
        f"sweep_limited={'YES' if boundary_status['limited'] else 'no'})"
    )
...
sweep_boundary_limited = np.array(
    [
        per_voltage[float(v)]["sweep_boundary_status"]["limited"]
        for v in envelope_values
    ],
    dtype=bool,
)
upper_sweep_limited = np.array(
    [
        per_voltage[float(v)]["sweep_boundary_status"]["upper_limited"]
        for v in envelope_values
    ],
    dtype=bool,
)
lower_sweep_limited = np.array(
    [
        per_voltage[float(v)]["sweep_boundary_status"]["lower_limited"]
        for v in envelope_values
    ],
    dtype=bool,
)
...
base_fit_mask = (
    (np.abs(envelope_values) >= ABS_FIT_MIN_V)
    & fit_success
    & np.isfinite(peak_freq_Hz)
)
upper_limited_magnitudes = np.abs(
    envelope_values[base_fit_mask & upper_sweep_limited]
)
first_upper_limited_abs_voltage_V = (
    float(np.min(upper_limited_magnitudes))
    if len(upper_limited_magnitudes)
    else np.nan
)
calibration_fit_mask = base_fit_mask & ~lower_sweep_limited
if np.isfinite(first_upper_limited_abs_voltage_V):
    calibration_fit_mask &= (
        np.abs(envelope_values) < first_upper_limited_abs_voltage_V
    )

fit_min_included_abs_voltage_V = (
    float(np.min(np.abs(envelope_values[calibration_fit_mask])))
    if np.any(calibration_fit_mask)
    else np.nan
)
fit_max_abs_voltage_V = (
    float(np.max(np.abs(envelope_values[calibration_fit_mask])))
    if np.any(calibration_fit_mask)
    else np.nan
)

calibration_exclusion_reasons = []
for idx, voltage in enumerate(envelope_values):
    reasons = []
    if abs(voltage) < ABS_FIT_MIN_V:
        reasons.append(f"|V_env| < {ABS_FIT_MIN_V:g} V")
    if not fit_success[idx] or not np.isfinite(peak_freq_Hz[idx]):
        reasons.append("peak fit failed")
    if lower_sweep_limited[idx] and abs(voltage) >= ABS_FIT_MIN_V:
        reasons.append("peak reaches the lower sweep boundary")
    if (
        np.isfinite(first_upper_limited_abs_voltage_V)
        and abs(voltage) >= first_upper_limited_abs_voltage_V
    ):
        reasons.append(
            "true peak reaches or exceeds the upper sweep boundary from "
            f"|V_env|={first_upper_limited_abs_voltage_V:g} V"
        )
    calibration_exclusion_reasons.append("; ".join(reasons) if reasons else None)

signed_fit = linear_fit(
    envelope_values[calibration_fit_mask], peak_freq_Hz[calibration_fit_mask]
)
positive_mask = calibration_fit_mask & (envelope_values >= ABS_FIT_MIN_V)
negative_mask = calibration_fit_mask & (envelope_values <= -ABS_FIT_MIN_V)
abs_mask = calibration_fit_mask
        ...
        pair_rows.append(
            {
                ...
                "included_in_calibration_fit": bool(
                    calibration_fit_mask[pos[0]] and calibration_fit_mask[neg[0]]
                ),
            }
        )
...
ax1.errorbar(
    ...
    yerr=np.where(
        np.isfinite(peak_unc_Hz[calibration_fit_mask]),
        peak_unc_Hz[calibration_fit_mask],
        0.0,
    ),
    fmt="o",
    color="C0",
...
excluded_fit_mask = ~calibration_fit_mask
if np.any(excluded_fit_mask):
    ax1.errorbar(
        envelope_values[excluded_fit_mask],
        peak_freq_Hz[excluded_fit_mask],
        yerr=np.where(
            np.isfinite(peak_unc_Hz[excluded_fit_mask]),
            peak_unc_Hz[excluded_fit_mask],
            0.0,
        ),
        fmt="x",
        color="0.5",
...
fit_grid_mask = (
    np.abs(x_grid) <= fit_max_abs_voltage_V
    if np.isfinite(fit_max_abs_voltage_V)
    else np.zeros_like(x_grid, dtype=bool)
)
if absolute_fit["success"]:
    ax1.plot(
        x_grid[fit_grid_mask],
    ...
    fit_annotation = (
        ...
        "\n"
        f"Fit: {fit_min_included_abs_voltage_V:g} <= |V| <= {fit_max_abs_voltage_V:g} V"
    )
    ax1.text(
        0.98,
        0.50,
...
def fit_summary_for_voltage(voltage, result):
    fit = result["primary_fit"]
    idx = int(np.where(np.isclose(envelope_values, voltage))[0][0])
    return {
        ...
        "sweep_boundary_limited": bool(sweep_boundary_limited[idx]),
        "upper_sweep_boundary_limited": bool(upper_sweep_limited[idx]),
        "lower_sweep_boundary_limited": bool(lower_sweep_limited[idx]),
        "included_in_calibration_fit": bool(calibration_fit_mask[idx]),
        "calibration_exclusion_reason": calibration_exclusion_reasons[idx],
...
analysis = {
    ...
    "calibration_point_filter": {
        "method": (
            "exclude failed/lower-boundary fits; after the first |V_env| whose peak "
            "reaches the upper sweep edge, exclude that magnitude and all larger magnitudes"
        ),
        "sweep_edge_margin_steps": SWEEP_EDGE_MARGIN_STEPS,
        "minimum_abs_voltage_threshold_V": ABS_FIT_MIN_V,
        "first_upper_limited_abs_voltage_V": (
            first_upper_limited_abs_voltage_V
            if np.isfinite(first_upper_limited_abs_voltage_V)
            else None
        ),
        "fit_abs_voltage_range_V": (
            [fit_min_included_abs_voltage_V, fit_max_abs_voltage_V]
            if np.isfinite(fit_min_included_abs_voltage_V)
            and np.isfinite(fit_max_abs_voltage_V)
            else None
        ),
        "included_voltage_points_V": [
            float(value) for value in envelope_values[calibration_fit_mask]
        ],
        "excluded_voltage_points": [
            {
                "envelope_voltage_V": float(envelope_values[idx]),
                "reason": calibration_exclusion_reasons[idx],
            }
            for idx in np.where(~calibration_fit_mask)[0]
        ],
    },
    ...
    "calibration_models": {
        ...
        "positive_branch": {
            "formula": "f_peak_Hz = K_pos * V_env + B_pos, valid positive sweep range",
            **clean_fit(positive_fit),
        },
        "negative_branch_abs_axis": {
            "formula": "f_peak_Hz = K_neg_abs * abs(V_env) + B_neg, valid negative sweep range",
        ...
        "absolute_voltage_primary": {
            ...
            "fit_actual_min_abs_voltage_V": (
                fit_min_included_abs_voltage_V
                if np.isfinite(fit_min_included_abs_voltage_V)
                else None
            ),
            "fit_max_abs_voltage_V": (
                fit_max_abs_voltage_V if np.isfinite(fit_max_abs_voltage_V) else None
            ),
    ...
    "interpretation_note": (
        ...
        "靠近屏蔽带的点仍标记为低可信度。触及扫频上边界的 |V_env| 及更高"
        "电压点不参与标定拟合，避免将边界或次峰误认为真实峰。"
    ),
}
...
np.savez(
    ...
    peak_sweep_boundary_limited=sweep_boundary_limited,
    peak_included_in_calibration_fit=calibration_fit_mask,
    unfiltered_peak_frequency_Hz=unfiltered_peak_freq_Hz,
    fixed_spur_bands_Hz=np.asarray(FIXED_SPUR_BANDS_HZ, dtype=float),
    first_upper_limited_abs_voltage_V=first_upper_limited_abs_voltage_V,
